...
def rbf(x_1, x_2, sigma=1.):
    ...
    x1_ = x_1.numpy()
    x2_ = x_2.numpy()
    dists = np.sum(x1_ ** 2, axis=1)[:, np.newaxis] - 2 * np.dot(x1_, x2_.T) + np.sum(x2_ ** 2, axis=1)
    distances = np.exp(-dists / (2 * (sigma ** 2)))
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def linear(x_1, x_2):
        ...
        '''
        return torch.Tensor(torch.matmul(x_1, x_2.T)).type(torch.float32)
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, n_obj, self.batch_size):
                ...
                preds = torch.matmul(k_batch, self.betas) + self.bias
                preds = preds.flatten()
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            scores = torch.matmul(K, self.betas) + self.bias
